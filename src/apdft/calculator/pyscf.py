...
class PyscfCalculator(apc.Calculator):
    ...
    def get_input(
        ...
        env['deltaZ'] = PyscfCalculator._format_list(np.array(nuclear_charges) - np.array(nuclear_numbers))
    ...
    def get_epn(folder, coordinates, includeatoms, nuclear_charges):
        ...
        if not set(included_results) != set(includeatoms):
            log.log('Atom selections do not match. Likely the configuration has changed in the meantime.', level='error')
