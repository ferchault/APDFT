...
def uhf(distance,pseudo_numbers,basis_set='cc-pvtz',concatenate=False):
    natoms=np.array([7,7] )
...
def b3lyp(distance,pseudo_numbers,basis_set='cc-pvtz',concatenate=False):
...
def rhf(distance,pseudo_numbers,basis_set='cc-pvtz',concatenate=False):
    natoms=np.array([7,7] )
    coordinates=np.array([[-distance/2.,0.,0.],[distance/2.,0.,0.]])
    mol=IOData(title='NN')
    mol.numbers=natoms
