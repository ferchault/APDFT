...
def get_times(input_file_prefix, input_file_postfix):
    #ZINC_named smallest molecule with 4 heavy atoms, largest with 59
    N = np.array([i for i in range(4,60)], dtype='int') #Number of atoms
    ...
    for log_num in range(1,last_log_num+1):
        ...
        for line in Lines:
            x = line.split('\t')
            num_moles[int(x[2])-4] += 1
            times[(int(x[2])-4)] += float(x[1])
            if float(x[1]) > max_time:
                max_time = float(x[1])
                max_time_index = x[0]
            times_variance[(int(x[2])-4)] += float(x[1])**2
        f.close()
    #print(num_moles)
    times /= num_moles
...
def get_CDF(input_file_prefix, input_file_postfix, bin_size = 50):
    N = np.array([i for i in range(4,60)], dtype='int') #Number of atoms    num_moles = np.zeros((8), dtype='int') #Number of times a molecule with N atoms occurs
    max = 0
    for log_num in range(1,last_log_num+1):
        f = open('logs/'+input_file_prefix+ '00'[:(2-len(str(log_num)))] + str(log_num) +input_file_postfix+'_geom.txt', 'r')
        Lines = f.readlines()
    ...
    for log_num in range(1,last_log_num+1):
        ...
        for line in Lines:
            ...
            if int(x[3]) == 0: #0 is a special bin
                num_AE[int(x[2])-4][0] += 1
            else:
                num_AE[int(x[2])-4][int(float(x[3])/bin_size)+1] += 1
                if int(x[3]) > 300:
                    print(x)
        f.close()
...
def average_num_AE(input_file_prefix, input_file_postfix):
    N = np.array([i for i in range(4,60)], dtype='int') #Number of atoms
    num_moles = np.zeros((len(N)), dtype='int') #Number of times a molecule with N atoms occurs
    num_moles_zero = np.zeros((len(N)), dtype='int') #Number of times a molecule with N atoms occurs
    ...
    for log_num in range(1,last_log_num+1):
        ...
        for line in Lines:
            x = line.split('\t')
            num_AE[int(x[2])-4] += float(x[3])
            num_moles[int(x[2])-4] += 1
            if float(x[3]) == 0:
                num_moles_zero[int(x[2])-4] += 1
    ...
    for log_num in range(1,last_log_num+1):
        ...
        for line in Lines:
            x = line.split('\t')
            num_AE_SD[int(x[2])-4] += (float(x[3])-num_AE[int(x[2])-4])**2
    for thing in N:
        #print(num_AE_SD)
        num_AE_SD[thing-4] = math.sqrt(num_AE_SD[thing-4]/num_moles[thing-4])
    #print('Percentage: '+str(num_AE*100))
...
NN, av, av_SD = average_num_AE('ZINC_log', '_dZ1')
#print(av)
#print(av_SD)
##N, times_dZ2, SD_dZ2 = get_times('ZINC_log', '_dZ2')
#N, times_dZ1, SD_dZ1 = get_times('ZINC_log', '_dZ1')


#------------------------Plot the average num_AE per N--------------------------
#We plot only 4 to 21, the rest is zero
fig, ax = plt.subplots()
ax.scatter(NN[:17], av[:17], marker='x', color=color[1], label='dZ_max = 1')
#plt.errorbar(NN[:17], av[:17], yerr=av_SD[:17], fmt='none', capsize=4, color=color[1])
ax.set_xticks(range(4,21))
ax.set_xlim([3.5, 20.7])
ax.set_ylim([0,120])
#plt.yscale('log')
...
fig.savefig("figures/ZINC_averagenumAE_dZ1_geom.png", dpi=500)
...
ax.scatter([2,3,4,5,6,7,8,9], QM9_percentages_uncolored_dZ1, marker='x', color=color[2], label='dZ_max = 1 (QM9, i.a.)')
ax.plot([2,3,4,5,6,7,8,9], QM9_percentages_uncolored_dZ1,color=color[2])
ax.scatter([2,3,4,5,6,7,8,9], QM9_percentages_uncolored_dZ2, marker='x', color=color[3], label='dZ_max = 2 (QM9, i.a.)')
