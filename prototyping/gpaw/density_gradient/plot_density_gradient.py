...
# laod GPAW object   
calc_obj = GPAW(r'/home/misa/APDFT/prototyping/gpaw/OFDFT/results/gpw_geometry/result_64_gpts_d_1.08.gpw') # load data from SCF calculation
...
### get density
ps_dens = calc_obj.get_pseudo_density(pad=False)*Bohr**3 # convert to atomic units
constrain = np.sum(ps_dens) # ensure that integral of pseudo density is constant
sqrt_ps_dens = np.sqrt(ps_dens) # take sqrt of pseudo density to get "orbital" \sqrt{\tilde{\rho}}


### construct dE/drho
# construct kinetic energy gradient
kin_operator = np.zeros(sqrt_ps_dens.shape, dtype=float) # create grid for kinetic energy operator applied to orbital
calc_obj.wfs.kin.apply(sqrt_ps_dens, kin_operator, phase_cd=None) # calculate \hat{T} \tilde{\rho}
# construct effective potential
eff_potential = calc_obj.get_effective_potential(pad=False)/Hartree # effective potential of pseudo density (including kinetic energy functional thomas-fermi...) in atomic units
# build density gradient \partial E / \partial \rho  ( see J. Chem. Phys. 141, 234102 (2014); eq.(3) )
dens_grad = kin_operator/sqrt_ps_dens + eff_potential
