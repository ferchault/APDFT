...
reps = [build_rep_global(f"{_}/POSCAR", symmetrize=False) for _ in tqdm.tqdm(folders)]
# reps2 = [
...
def get_KRR_learning_curve(reps, Y, k=5):
    ...
    D = skm.pairwise_distances(reps, n_jobs=os.cpu_count())
    Kall = np.array([np.exp(-(D ** 2) / (2 * _ * _)) for _ in allsigmas])
