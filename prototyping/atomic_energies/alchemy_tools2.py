...
def ewald_site(a, mol):
    """
    calculates the nuclear repulsion for a molecule within periodic boundary conditions and returns the nuclear repulsion per site
    a: boxsize (asssuming cubic box)
    mol: molecule as an ase.Atoms object
    """
...
def get_idc_bl_grid_in_box(grid_coords, box_param):
    """
    returns the indices of the grid coordinates of Becke-Lebedev grid that are in rectangular box specified by box_param
    grid_coords: the coordinates of the Becke-Lebedev grid; numpy array shape (num grid points, 3)
    box_param: start and end point of box in x,y,z direction; tuple/list of shape ((x_min, x_max), (y_min, y_max), (z_min, z_max))
    """
    idc_ax = []
    for p, ax in zip(box_param, [0,1,2]):
        idc_ax.append( np.where(( p[0] <grid_coords[:,ax]) & (grid_coords[:,ax] < p[1])) )

    intersection = np.intersect1d(idc_ax[0], idc_ax[1])
    intersection = np.intersect1d(intersection, idc_ax[2])
    return(intersection)
