...
int main(int argc, char ** argv) {
	...
	for (size_t i = begin; i < end; i++) {
		...
		GFN1_calculation(&natoms, attyp, &charge, &coord, opt, "-", &energy, grad);
		stream << "energy" << energy << std::endl;
		stream << "grad" << grad << std::endl;
		std::cout << stream.str();
	}
